...
def find_unused_elbs(region):
    ...
    for load_balancer in response['LoadBalancerDescriptions']:
        tags = elb_client.describe_tags(LoadBalancerNames=[load_balancer['LoadBalancerName']])
        ...
        if len(instances['InstanceStates']) == 0:
            ...
            elb_client.delete_load_balancer(LoadBalancerName=load_balancer['LoadBalancerName'])
    return unused_elbs
...
# Function to Format message to Slack
        ...
        for region, elbs in data.items():
            if not elbs:
                ...
                for elb in elbs:
                    ...
                    tags = elb_client.describe_tags(ResourceArns=[elb])['TagDescriptions'][0]['Tags']
                    protected = False
                    for tag in tags:
                        if tag['Key'] == "protected" and tag['Value'] == "true":
                            ...
                            break
                    if protected:
                        message += f"```ALB `{elb}` has the tag \"protected\": \"true\"```"
                    else:
                        message += f"```{elb}```\n"
                message += "\n"
    return message
