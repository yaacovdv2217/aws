...
`{2}`
            ...
            if instance_dict["status"] != "terminated":
                slack_message(msg_in_case_valid, None,
                              "https://hooks.slack.com/services/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")
...
def slack_message(message_text=None, payload_obj=None, slack_url=None):
    ...
    if (not payload_obj and message_text):
        payload_obj = {"text": "\n" + str(message_text) + "\n", "username": "NOT TERMINATING", "link_names": 1,
                       "icon_emoji": ":monkey:"}
    ...
    for region in instances_to_terminate:
        ...
        if response_termination:
            ...
            payload_obj = {"text": "```\n" + str(response_termination_final) + "\n```",
                           ...
                           "icon_emoji": ":gun:"}
            if response_termination != "":
                slack_message(None, payload_obj=payload_obj,
                              slack_url="https://hooks.slack.com/services/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")
                payload_obj = {}
...
def update_global_dict(instance_dict, region):
    ...
    try:
        instance_dictionary["tags"] = [
            {"creator": check_if_creator_exist_and_update(instance_dict["Instances"][0]["Tags"])},
            {"date": check_if_date_exist_and_update(instance_dict["Instances"][0]["Tags"])}]
    except Exception as e:
