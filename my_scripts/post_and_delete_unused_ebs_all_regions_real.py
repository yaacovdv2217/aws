...
def describe_unused_ebs(region):
    ...
    for volume in regional_client.volumes.filter(
        ...
        if volume.state == "available":
            ...
            print("ID:", volume.id, "Status: available")
    return volumes
...
def format_delete_message(unused_ebs_dict):
    message = f"*Start Deleting Unused EBS Volumes in {selected_profile}*\n"
    for region, volumes in unused_ebs_dict.items():
        message += f"*{region}*\n"
        for volume in volumes:
            message += f"{volume} - deleted\n"
    return message
...
def delete_unused_ebs(region):
    ...
    for volume in regional_client.volumes.filter(
        ...
        if volume.state == "available":
            ...
            volumes.append(volume_id)
            volume.delete()
            print("ID:", volume.id, "Status: deleted")
    return volumes
...
def main2():
    message2 = format_delete_message(delete_ebs_in_all_regions())
    send_message(message=message2)
